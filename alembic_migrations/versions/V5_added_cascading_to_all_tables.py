...
def upgrade() -> None:
    ...
    op.alter_column("broker_clients", "access_token", existing_type=sa.TEXT(), nullable=False)
    op.alter_column("broker_clients", "twoFA", existing_type=sa.INTEGER(), nullable=False)
    op.drop_constraint("broker_user_id_fkey", "broker_clients", type_="foreignkey")
    op.create_foreign_key(None, "broker_clients", "user", ["user_id"], ["id"], ondelete="CASCADE")
    ...
    op.create_foreign_key(
        None, "strategy", "broker_clients", ["broker_id"], ["id"], ondelete="CASCADE"
    )
...
def downgrade() -> None:
    ...
    op.create_foreign_key(
        "strategy_broker_id_fkey", "strategy", "broker_clients", ["broker_id"], ["id"]
    )
    op.drop_constraint(None, "daily_profit", type_="foreignkey")
    op.create_foreign_key(
        "daily_profit_strategy_id_fkey", "daily_profit", "strategy", ["strategy_id"], ["id"]
    )
    op.drop_constraint(None, "broker_clients", type_="foreignkey")
    op.create_foreign_key("broker_user_id_fkey", "broker_clients", "user", ["user_id"], ["id"])
    op.alter_column("broker_clients", "twoFA", existing_type=sa.INTEGER(), nullable=True)
    op.alter_column("broker_clients", "access_token", existing_type=sa.TEXT(), nullable=True)
    # ### end Alembic commands ###
