...
async def test_trading_nfo_options_first_ever_trade_for_short_strategy(
    ...
    async with Database() as async_session:
        ...
        assert (
            trade_db_model.option_type == OptionType.CE
...
async def test_trading_nfo_options_opposite_direction_for_short_strategy(
    ...
    async with Database() as async_session:
        ...
        redis_trade_list_json = await test_async_redis_client.hget(
            str(strategy_db_model.id),
            f"{exited_trade_db_models[0].expiry} {OptionType.CE if action == SignalTypeEnum.SELL else OptionType.PE}",
        )
        ...
        actual_future_profit = sum(
            trade_db_model.future_profit for trade_db_model in exited_trade_db_models
        )
        ...
        assert strategy_db_model.future_funds == round(old_future_funds + actual_future_profit, 2)
