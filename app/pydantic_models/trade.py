...
class SignalPydModel(BaseModel):
    ...
    future_entry_price_received: float = Field(
        description="Future Entry Price", examples=[40600.5]
    )
    strategy_id: uuid.UUID = Field(
        description="Strategy ID", examples=["ff9acef9-e6c4-4792-9d43-d266b4d685c3"]
    )
    received_at: datetime = Field(
        description="Received At", examples=["2023-05-22 05:11:01.117358"]
    )
    action: SignalTypeEnum = Field(description="buy or sell signal", examples=["buy"])
    strike: Optional[float] = Field(description="Strike", examples=[42500.0], default=None)
    ...
    expiry: Optional[date] = Field(description="Expiry", examples=["2023-06-16"], default=None)
    quantity: Optional[int] = Field(description="Quantity", examples=[15], default=0)
...
class RedisTradePydModel(SignalPydModel):
    ...
    id: uuid.UUID = Field(
        description="Trade ID", examples=["ff9acef9-e6c4-4792-9d43-d266b4d685c3"]
    )
    strike: Optional[float] = Field(description="Strike", examples=[42500.0], default=None)
    entry_price: Optional[float] = Field(
        description="Entry Price", examples=[350.5], default=None
    )
    expiry: date = Field(description="Expiry", examples=["2023-06-16"])
    instrument: str = Field(description="Instrument", examples=["BANKNIFTY16JUN2343500CE"])
    ...
    symbol: Optional[str] = Field(description="Symbol", examples=["BANKNIFTY"], default="")
    received_at: Optional[datetime] = Field(
        description="Received At", examples=["2023-05-22 05:11:01.117358"], default=None
    )
    action: SignalTypeEnum = Field(description="buy or sell signal", examples=["buy"])
    quantity: int = Field(description="Quantity", examples=[15])
...
class ExitTradePydModel(BaseModel):
    ...
    exit_price: float = Field(description="Exit Price", examples=[450.5])
    profit: float = Field(description="Profit", examples=[2500.0])
    future_exit_price_received: float = Field(description="Future Exit Price", examples=[40700.5])
    future_profit: float = Field(description="Future Profit", examples=[2500.0])
    exit_at: datetime = Field(
        description="Exited At",
        default=datetime.utcnow(),
        examples=["2023-05-22 06:25:03.117358"],
    )
    exit_received_at: datetime = Field(
        description="Received Exit At", examples=["2023-05-22 06:25:03.117358"]
    )
...
class EntryTradePydModel(SignalPydModel):
    ...
    future_entry_price: Optional[float] = Field(
        description="Future Entry Price", examples=[40600.5], default=None
    )
    quantity: int = Field(description="Quantity", examples=[15])
    entry_at: datetime = Field(
        description="Placed At",
        default_factory=datetime.utcnow,
        examples=["2023-05-22 05:11:04.117358+00"],
    )
    entry_received_at: datetime = Field(
        description="Received At", examples=["2023-05-22 05:11:01.117358"]
    )

    expiry: date = Field(description="Expiry", examples=["2023-05-22"])
    instrument: str = Field(description="Instrument name", examples=["BANKNIFTY27APR23FUT"])
    action: SignalTypeEnum = Field(description="buy or sell signal", examples=["buy"])
...
class OptionsEntryTradePydModel(FuturesEntryTradePydModel):
    model_config = ConfigDict(from_attributes=True)
    strike: float = Field(description="Strike", examples=[42500.0], default=None)
...
class DBEntryTradePydModel(BaseModel):
    ...
    quantity: int = Field(description="Quantity", examples=[25])
    strategy_id: uuid.UUID = Field(
        description="Strategy ID", examples=["ff9acef9-e6c4-4792-9d43-d266b4d685c3"]
    )
    option_type: OptionTypeEnum = Field(
    ...
    instrument: str = Field(description="Instrument", examples=["BANKNIFTY16JUN2343500CE"])
    action: SignalTypeEnum = Field(description="Buy or Sell Action", examples=["buy"])
    entry_price: float = Field(description="Entry Price", examples=[350.5])
    future_entry_price: float = Field(description="Future Entry Price", examples=[40600.5])
    future_entry_price_received: float = Field(
        description="Future Entry Price", examples=[40600.5]
    )
    entry_received_at: datetime = Field(
    ...
    strike: float = Field(description="Strike", examples=[42500.0])
    expiry: date = Field(description="Expiry", examples=["2023-05-22"])
...
class CFDPayloadPydModel(BaseModel):
    ...
    direction: SignalTypeEnum = Field(description="buy or sell signal", examples=["buy"])
...
class BinanceFuturesPayloadPydModel(BaseModel):
    model_config = ConfigDict(from_attributes=True)
    symbol: str = Field(description="Symbol", examples=["BTCUSDT"])
    quantity: float = Field(description="Quantity", examples=[1])
    side: SignalTypeEnum = Field(description="buy or sell signal", examples=["buy"])
    type: str = Field(description="Type", examples=["MARKET"])
    ltp: float = Field(description="LTP", examples=["27913"])
